...
struct compiler_unit {
    ...
    */
    ...
    PyObject *u_privatevars;	/* private variables */
...
static int compiler_addop_p(struct compiler *, int, PyObject *);
...
static int compiler_isPrivate(struct compiler *, char *);
                     ...
                     int lineno)
{
    ...
    if(u->u_ste->ste_type == ClassBlock){
    	u->u_privatevars = PyDict_New();
    }
    else {
    	u->u_privatevars = Py_None;
    }
...
/*
 * adding private opcodes to compiler
 * keeping them separate from regular names.
 */
static int
compiler_addop_p(struct compiler *c, int opcode, PyObject *name){

	int arg;

    /* previous unit, i.e the class */
    int n;
    PyObject *capsule;
    struct compiler_unit *u;
    n = PyList_GET_SIZE(c->c_stack) - 1;
    ...
    if (n >= 0) {
    	...
    	if (u->u_ste->ste_type != ClassBlock){
    		/* parent is not ClassBlock*/
    		return compiler_error(c, "Parent is not able to store privates.");
    	}
    }
    else{
    	/* No stack, top level */
    	return compiler_error(c, "Privates can't be accessed from top-level.");
    }

	arg = compiler_add_o(c, u->u_privatevars, name);
	if (arg < 0)
	  return compiler_error(c, "Error storing private name.");
...
compiler_class(struct compiler *c, stmt_ty s)
{
    ...
    PyObject* nprivates_o = PyInt_FromLong(PyDict_Size(c->u->u_privatevars));
    ...
    ADDOP_O(c, LOAD_CONST, nprivates_o, consts);
...
/*
 * Check whether an attribute is private
 * returns 1 if attribute is private
 * returns 0 otherwise
 */
static int
compiler_isPrivate(struct compiler *c, char *name)
{
	/*
	 * privates exists only within FunctionBlocks embedded in ClassBlocks
	 *
	 * The name of the attribute is must start with "private_" which is
	 * 8 characters long. (yes, "private_" will fail this, "private_a" won't)
	 */

	/*
	 * The following code is taken and modified from
	 * compiler_exit_scope() in compiler.c
	 */
	int n;
	PyObject *capsule;
	struct compiler_unit *u;
	...
	if(n >= 0) {
		...
		u = (struct compiler_unit *)PyCapsule_GetPointer(capsule, COMPILER_CAPSULE_NAME_COMPILER_UNIT);
		if (u->u_ste->ste_type != ClassBlock){
			/* The parent unit is not a class block */
			return 0;
		}
	}
	else{
	 ...
	 */
	if(PyString_Size(PyString_FromString(name)) > 8
			&& strncmp(name, "private_", 8) == 0){
		return 1;
	}
    return 0;
}
...
compiler_nameop(struct compiler *c, identifier name, expr_context_ty ctx)
{
    ...
    case OP_NAME:
        ...
        case Store:
        	op = STORE_NAME;
            if(c->u->u_ste->ste_type == ClassBlock && compiler_isPrivate(c, PyString_AsString(mangled))){
            	op = STORE_PRIVATE;
            }
            if(c->u->u_ste->ste_varnames){  // <Prem>
            	printf("Variables : %d",PyList_Size(c->u->u_ste->ste_varnames));
            }
            break;
...
compiler_visit_expr(struct compiler *c, expr_ty e)
{
    ...
    */
    ...
    case Attribute_kind:
    	...
    	case Load:
    		if(compiler_isPrivate(c, PyString_AsString(e->v.Attribute.attr))){
    			compiler_addop_p(c, LOAD_PRIVATE, e->v.Attribute.attr);
    		}
    		else{
    	...
    	case Store:
    		if(compiler_isPrivate(c, PyString_AsString(e->v.Attribute.attr))){
    			compiler_addop_p(c, STORE_PRIVATE, e->v.Attribute.attr);
    		}
    		else{
